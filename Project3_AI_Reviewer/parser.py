...
# how the response from Scraper gets processed. The setSoup() allows for the Parser's soup variable to be
...
class Parser:
    ...
    def __init__(self):
        #takes the Response object and puts it in soup so its in a structured format.
        self.soup = "None"
        self.reviews = []
    ...
    def getReviews(self):
        ...
        #gets each review from that people left for a product on eBay
        for review in reviewsFromHTML:
            ...
            # Ensure the review is a single line by removing newlines and extra spaces via split()
            singleLineReview = ' '.join(userReview.split())  
        
            # Append the review that is now on a one line
            self.reviews.append(singleLineReview)